...
class Smells(BaseModel):
    smells: List[Smell]

def code_smells_agent(code_changes):
    prompt = f"""
Analyze the provided code changes to a github repository in form of git diffs with some context for code smells. 
Read https://refactoring.guru/refactoring/smells and use it as references to identify potential smells.

For each detected smell, include:
- Name of Code Smell
- Explanation of why it is a code smell and its potential impact.
...
Some other code smells:
- Hidden Mine: code changes that might break other codes which are dependent on these functions
- Bad Words: incomplete code, indicated by TODOs and WIPs
- Unprofessional: console logs and print statements
...
"""
    ...
    try:
        fin = Smells(**json.loads(response[7:-3]))
        return fin
    except:
        print(f"Failed at Code Smells Agent")
...
if __name__ == "__main__":
    ...
    response = code_smells_agent([change2, change])
    print(response)